...
ResultCode Mutex::Release(VAddr address, Thread* holding_thread) {
    ...
    auto [thread, num_waiters] =
        GetHighestPriorityMutexWaitingThread(SharedFrom(holding_thread), address);
    ...
    TransferMutexOwnership(address, SharedFrom(holding_thread), thread);
