...
static void SetCompressorEffectParameter(CompressorInfo::ParameterVersion2& params,
    ...
    const auto c{static_cast<f32>(((b * 1.0f) + 1.0001f) + std::pow(b, 2) * 0.49912f +
                                  std::pow(b, 3) * 0.16653f + std::pow(b, 4) * 0.043591f +
                                  std::pow(b, 5) * 0.0086169f)};
    ...
    if (enabled) {
        ...
        for (u32 i = 0; i < sample_count; i++) {
            ...
            if (b >= state.unk_10) {
                ...
                c = static_cast<f32>(((f * 1.0f) + 1.0001f) + std::pow(f, 2) * 0.49912f +
                                     std::pow(f, 3) * 0.16653f + std::pow(f, 4) * 0.043591f +
                                     std::pow(f, 5) * 0.0086169f);
            }
