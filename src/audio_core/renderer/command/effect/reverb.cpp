...
static void ApplyReverbEffect(const ReverbInfo::ParameterVersion2& params, ReverbInfo::State& state,
    ...
    for (u32 sample_index = 0; sample_index < sample_count; sample_index++) {
        ...
        for (u32 channel = 0; channel < NumChannels; channel++) {
            ...
            Common::FixedPoint<50, 14> out_sample{};

            if ((NumChannels == 6) && (channel == static_cast<u32>(Channels::Center))) {
                const auto center{state.center_delay_line.Tick(
                    (out_line_samples[2] - out_line_samples[3]) * 0.5f)};
                out_sample = ((output_samples[channel] + center) * wet_gain) / 64;
            } else {
                out_sample =
                    ((output_samples[channel] + out_line_samples[channel]) * wet_gain) / 64;
            }
